...
|----|------|--------|----------------|
| G3 | Complete production deploy sign-off against the real public URL | Done | The workflow-driven exact-SHA deployment, pre-deploy backup, PostgreSQL migration, readiness, non-mutating sign-off, and automatic rollback path are operational. Phase 5 production deployed successfully in [workflow run 30055808149](https://github.com/James80401/PlantCare/actions/runs/30055808149). See [Phase 2 evidence](../operations/release-evidence-2026-07-23-phase-2.md) and [production-signoff.md](../operations/production-signoff.md). |
...
|----|------|--------|-------|
| H1 | Slim `GET /dashboard` and add cross-plant journal/diagnosis summaries | Done | The dashboard uses bounded field-level selects, embeds `gardenSummaries`, performs no GET-side maintenance writes, and avoids the duplicate client garden request. PostgreSQL fixtures measured the 100-plant response at 36,110 bytes and 22.3 ms warmed p95. See [Phase 5 evidence](../operations/release-evidence-2026-07-23-phase-5.md). |
...
| INTEL-1 | Expand plant problem, treatment, and guide intelligence | Next - analytics shipped | **First pass shipped (2026-06-30):** curated plant problem library, care archetypes, versioned treatment-plan builder, treatment-plan-first recovery suggestions, and Health tab treatment-plan UI. **Guide analytics shipped (2026-07-02):** diagnosis guide links emit `guide_link_click`; plant care-card Dr. Plant CTAs emit `guide_dr_plant_click`. Track remaining catalog expansion, hybrid long-tail identification, and guide intelligence in [plant-intelligence-expansion.md](plant-intelligence-expansion.md). |
| PLANTLIFE-1 | Polish per-plant progress history | Done | **Plant Life polish shipped and extended (2026-07-03):** progress entries can be edited/deleted, AI progress stories refresh after edits, health-check tasks deep-link into check-ins, and the Journal tab shows latest story, trend, persisted milestones, photos, and history controls. Per-plant milestones now persist for baseline check-ins, three check-ins, progress photos, new growth, stable streaks, and recovery signals. |
| TASKTYPE-1 | Decide whether to support additional care task types | Done / Deferred expansion | **Reconciled 2026-07-04:** keep the shipped 12 task types. Shared task metadata now centralizes labels/prompts, API/web labels use that metadata, and task completion/skip prompts are task-type-aware. Harvest, move, and flush remain deferred product ideas, not active schema/API work, until scheduling behavior, guide coverage, feedback semantics, and Dr. Plant context are defined. |
| QUALITY-1 | Strengthen state and regression coverage | Next — in progress | Add or refresh tests for dashboard/API behavior, empty/loading/error states, warning/success contrast, and public preview/mobile workflow documentation. **Progress (2026-06-03):** enabled real component-state testing for web (`@testing-library/react` + jest-dom via `vitest.setup.ts`, previously unused) and added loading/error/empty/populated coverage for `DrPlantContextPanel` and the plant timeline type filters. Recent slices also added contract/regression tests (dashboard summaries, dashboard garden-summary retry/status, task feedback terminal-vs-snooze, species inference, Dr. Plant context builder). |
...
|----|------|--------|-------|
| JOURNAL-1 | Additional photo comparison and post-care observation prompts | Later / Reconcile | Growth measurements, photo comparison, task observation notes, and Plant Life milestones are shipped. Remaining future polish is optional: richer photo compare annotations or smarter post-care prompts if usage shows value. |
| ENGAGE-1 | Per-plant milestone expansion | Partial / Future polish | `PlantMilestone` now persists per-plant Plant Life milestones. Future expansion can add richer event types such as first leaf, first bloom, repot recovery, propagation, and pest cleared once we decide which moments are worth surfacing. |
| PHOTO-1 | Exact reusable images for final catalog gaps | Done | Exact Wikimedia Commons sources closed Hoya Mathilde (public domain) and String of Dolphins (CC BY-SA 4.0) on 2026-07-23. The catalog now verifies 447/447 local files and reusable-license records. See [species-photo-sourcing-log.md](species-photo-sourcing-log.md). |
...
| DRPLANT-1 | Chat actions that create tasks or recommendations | Done | Assistant replies can draft task/recommendation cards, and each card requires explicit user confirmation before a task or durable recommendation is created. Journal-note actions remain available separately from the chat action menu. |
