...
|----|------|--------|----------------|
| G3 | Complete production deploy sign-off against the real public URL | Now — deploy automation blocked | Public web is `https://drplant.app`; API base is `https://api.drplant.app/api/v1`. Live health + readiness probes pass (2026-06-04). **Blocker found 2026-06-04:** the `deploy-production.yml` GitHub Action fails at SSH auth — `DEPLOY_SSH_PRIVATE_KEY` is passphrase-protected but no `DEPLOY_SSH_PASSPHRASE` secret is set (`ssh: this private key is passphrase protected` → handshake fails). Production is live from an earlier/manual deploy; this workflow has never succeeded. **To unblock:** set the `DEPLOY_SSH_PASSPHRASE` repo secret (or replace the deploy key with a passphrase-less one), then re-run the workflow. Remaining after that: full `production:check`, `production:signoff`, optional production E2E, record a sign-off artifact. See [production-signoff.md](../operations/production-signoff.md) and [uat-checklist.md](uat-checklist.md#f-pre-release-for-remote-testers). |
...
| CONTENT-1 | Expand species catalog coverage by category | Next — metadata pass done | Older checklist still lists category expansion for houseplants, succulents/cacti, herbs/edibles, vegetables/fruits, ornamentals/shrubs/trees, and native/pollinator-friendly metadata. Keep separate from B3 attributes/filters, which newer docs mark done. **Metadata-quality pass shipped (2026-06-03)** — see [species-coverage-report.md](species-coverage-report.md): broadened the real inference so `highHumidity` 13→37 and `bloomsIndoors` 9→17 (and fixed an indoor-bloomer exclusion bug for succulent bloomers like Christmas cactus); the audit now calls the live inference so it can't drift. `lowLight` (10) is intentionally conservative — lifting it needs curated `sunlight` edits, not a heuristic. **Remaining:** optional curated `lowLight` edits and/or net-new species rows, both gated on the open "which categories matter most" product decision. |
