...
| CONTENT-1 | Expand species catalog coverage by category | Next | Older checklist still lists category expansion for houseplants, succulents/cacti, herbs/edibles, vegetables/fruits, ornamentals/shrubs/trees, and native/pollinator-friendly metadata. Keep separate from B3 attributes/filters, which newer docs mark done. **Coverage audit (2026-06-03, 321 species)** in [species-coverage-report.md](species-coverage-report.md): thin filter attributes are `bloomsIndoors` (9), `lowLight` (10), and `highHumidity` (13). These read as **under-tagging on existing species**, not missing content — a metadata-quality pass likely beats adding rows. |
